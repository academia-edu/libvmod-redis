...
static gboolean redisGlibSourceDispatch(GSource *source,
	...
	((void)user_data);
	redisGlibEvents *e = (redisGlibEvents*)source;
	gushort revents = e->poll_fd.revents;
	gboolean ret = TRUE;
...
int redisGlibAttach(GMainContext *ctx, redisAsyncContext *ac) {
	redisContext *c = &(ac->c);
	redisGlibEvents *e;
	...
	e = (redisGlibEvents*) g_source_new(&redisGlibSourceFuncs, sizeof(redisGlibEvents));
	e->context = ac;
...
int redisGlibDetach(redisAsyncContext *ac) {
	/* Nothing should be detached when nothing is already attached */
	if (ac->ev.data == NULL)
		return REDIS_ERR;

	g_source_destroy((GSource *) ac->ev.data);
